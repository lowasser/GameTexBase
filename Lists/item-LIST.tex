...
\NEW{Item}{\iTest}{
  \s\MYname	{The MacGuffin}
  %\s\MYnumber	{0000}
  \s\MYtext	{It must be very important to your character, but it's hard to tell what it actually is or does.}
}
...
\NEW{ItemFold}{\iTestFold}{
  \s\MYname	{Red Button Remote}
  \s\MYtext	{A remote control with a big red button.  Open packet if
		you press the big red button.}
  \s\MYcontents	{It beeps at you.}
}
...
\NEW{ItemLabel}{\iTestLabel}{
  \s\MYname	{Holy Grail}
  \s\MYtext	{This grail is very, very holy.}
}
