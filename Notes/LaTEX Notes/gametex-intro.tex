...
Note that bluesheets should always and only be assigned to characters.  Greensheets will almost exclusively go to characters, but may be placed elsewhere in game if characters can unlock new mechanics.  Whitesheets are essentially items, just items representing documents, and can go essentially anywhere.

\begin{subsubsection}{When to use greensheets}
When building mechanics, you have several options for how to present them to your players:
\begin{itemize}
...
In general, mechanics should usually go on greensheets if they are long, require specific tools for players to execute (such as dice or a deck of cards), or if players will likely want access to them before game.
\end{subsubsection}
\end{subsection}
\begin{subsection}{Power user features}
Not all games will want these features, but LARP authors should at least know that \gametex{} offers these features to be used.
\begin{subsubsection}{Abilities}
Abilities print ``cards'' that have a front and a back, with the front expected to contain full documentation about the ability, and the back containing the publicly visible effect of the ability.  
The definition of an ability might look like the following block in \lstinline|Lists/abil-LIST.tex|:
\begin{verbatim}
\NEW{Abil}{\aTest}{
  \s\MYname	{Ability Name} % can be cross-referenced
  \s\MYtext	{You are a test.} % on one side of the card, for the user to see
  \s\MYeffect	{This is a Test.} % on the other side of the card, for the target to see
}
\end{verbatim}
...
Abilities also print on handy cards, which can help players with many documents to keep track of their abilities more easily.
\end{subsubsection}
\begin{subsubsection}{Mempackets and membooks}\label{mems}
Mempackets and membooks are how \gametex{} handles secrets that players manage themselves.  As discussed earlier, an example mempacket might be assigned to the character Inigo Montoya.
Mempackets have triggers and contents: Inigo's mempacket might have the trigger ``Open after speaking to Count Rugen for one minute,'' and the content ``You notice Count Rugen has six fingers on his right hand.''
...
\item[MemCover/MemPage] An unordered collection of mem-folds, with a ``cover sheet'' with a title for the collection.  For example, a character who can read fortunes for other characters in game might have a ``Casting the Tarot'' cover for mempages with the fortunes for each character.
\end{desc}
...
Mempacket names cannot cross-reference other names, such as character names, but mempacket triggers can.
...
\NEW{MemEnvelope}{\mEnvTest}{
  ...
  \s\MYtext	{This is a little text strip that got tucked into a little envelope.}
}
...
\end{verbatim}
\end{subsubsection}
\begin{subsubsection}{Notebooks} \label{notebooks}
Notebooks are like a chain of dependent mempackets.  Usually, they look like a sequence of pages that can be torn open like a membook, but they have numbers instead of triggers.  Each page will usually say something like ``When you go do X, open page $N$.''

\begin{verbatim}
\documentclass[notebook]{gamename} %% [notebook] or [greennotebook]
\begin{document}
\startnotebook{\nTest{}}
...
To print your game, you will need to compile and print the relevant files from the \lstinline|Production| folder of your game.  There are a number of options for how to print your game, depending on who you are printing for at what stage in the development process.
...
edit \lstinline|LaTeX/gametex.sty| in the section labeled ``parts of namebadges.''
\end{subsection}
\end{section}
\end{document}