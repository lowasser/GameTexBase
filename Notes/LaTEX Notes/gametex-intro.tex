...
\end{subsection}
\begin{subsection}{Bluesheets and Greensheets}
As a reminder, bluesheets are documents used to provide in-universe lore known by more than one character, and greensheets are documents used to define mechanics from an out of character perspective.
Both are written essentially as freeform text.
...
Greensheets are analogous in the ways you expect: \lstinline|\documentclass[green]|, \lstinline|green-LIST.tex|, \lstinline|\s\MYgreens{...}|, and so on.

Note that bluesheets should always and only be assigned to characters.  Greensheets will almost exclusively go to characters, but may be placed elsewhere in game if characters can unlock new mechanics.
